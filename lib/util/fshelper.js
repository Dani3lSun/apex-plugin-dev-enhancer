...
const jsondir = require(appRootFolder + '/lib/util/jsondir/index.js');
...
module.exports = fshelper = {
  ...
  findFile: function(pPath, pSearchString, callback) {
    var walk = function(dir, done) {
      var results = [];
      fs.readdir(path.resolve(dir), function(err, list) {
  ...
  pathExists: function(pFilePath) {
    var exists = fse.pathExistsSync(path.resolve(pFilePath));
    return exists;
  },
  /**
   * Check if a file path exists
  ...
  /**
   * Get file list of of specified folder recursively with regexp pattern support
   * @param {string} pPath
   * @param {string} pRegExpPattern
   * @return {array}
   */
  getFileList: function(pPath, pRegExpPattern) {
    ...
    var walkSync = function(dir, filelist) {
      ...
      files.forEach(function(file) {
        if (fs.statSync(path.join(dir, file)).isDirectory()) {
          ...
          if (pRegExpPattern) {
            var regex = new RegExp(pRegExpPattern);
            if (regex.test(file)) {
