...
module.exports = function(args) {
  ...
  if (fshelper.pathExists(process.cwd() + '/apdeconfig.json')) {
    process.stdout.write('\n' + chalk.blue.bold('apdeconfig.json already present in project root!') + '\n');
    process.stdout.write('\nRun: ' + chalk.blue.bold('apde config') + ' to overwrite it!\n\n');
    process.exit();
  }
  ...
  prompt.get(schema, function(err, result) {
    if (result) {
      ...
      folderStructureContent = folderStructureContent.replace(/#working_dir#/g, JSON.stringify(process.cwd()).replace(/^"(.*)"$/, '$1'));
      try {
